...
// ---------- small data-URI placeholders (offline-safe) ----------
...
export default function MovieDetailPage() {
      ...
      try {
        ...
        if (!data || data.success === false) {
          ...
          if (item) {
            ...
            // Normalize poster/thumbnail/trailer etc. using getImageUrl
            if (item.poster) item.poster = getImageUrl(item.poster) || item.poster;
            if (item.thumbnail) item.thumbnail = getImageUrl(item.thumbnail) || item.thumbnail;
            if (item.img) item.img = getImageUrl(item.img) || item.img;
            if (item.posterUrl) item.posterUrl = getImageUrl(item.posterUrl) || item.posterUrl;
            ...
            ["cast", "directors", "producers"].forEach((k) => {
              if (Array.isArray(item[k])) {
                item[k] = item[k].map((p) => {
                  if (!p) return p;
                  const preview =
                    getImageUrl(p.preview || p.file || p.image) ||
                    (p.preview || p.file || p.image) ||
                    null;
                  return { ...p, preview, img: p.img ? getImageUrl(p.img) || p.img : preview };
                });
              }
            });
          }
  ...
  const getBookedCountFor = (datetime, audi = "Audi 1") => {
    try {
      ...
      if (bookedMap && typeof bookedMap[key] === "number") return bookedMap[key];
  ...
  // ensure posterSrc is normalized through getImageUrl
  const posterSrc = getImageUrl(movie.img || movie.thumbnail || movie.poster || movie.posterUrl) || PLACEHOLDER_POSTER;
  ...
  const producer =
    ...
    null;
  const producerImg = producer ? getImageUrl(producer.img || producer.preview || producer.file) : null;
              ...
              <div className={movieDetailHStyles.castGrid}>
                {movie.cast && movie.cast.length ? (
                  movie.cast.map((c, idx) => {
                    const src = getImageUrl(c.img || c.preview || c.file) || PLACEHOLDER_IMG_SM;
                    return (
                  ...
                  <div className={movieDetailHStyles.crewGridInner}>
                    {directors.length ? (
                      directors.slice(0, 2).map((d, i) => {
                        const src = getImageUrl(d?.img || d?.preview || d?.file) || PLACEHOLDER_IMG_MD;
                        return (
                          <div key={i} className="flex flex-col items-center">
                            <img
                              ...
                              onError={(e) => {
                                e.currentTarget.onerror = null;
                                e.currentTarget.src = PLACEHOLDER_IMG_MD;
                              }}
                            />
